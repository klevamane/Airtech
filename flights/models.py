...
class Flight(TimeStamp):
    ...
    source = models.ForeignKey(
        Airport, related_name="flights", on_delete=models.DO_NOTHING
    )
    destination = models.ForeignKey(
        Airport, related_name="destinations", on_delete=models.DO_NOTHING
    )
    ...
    seats = models.IntegerField(
        validators=[validate_seats],
        null=False,
        blank=False,
    )
    status = models.CharField(
        max_length=10, validators=[validate_status], default="active"
    )
